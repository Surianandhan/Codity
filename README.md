...
**A job surviving its worker being killed** — the centrepiece, recorded live. A worker claims a job
that takes longer than the lease, is killed with `SIGKILL` mid-flight, and a second worker finishes
it once the reaper reclaims the lease. The execution table at the end is read straight out of
Postgres: attempt 1 closed as `lost / LeaseExpired`, attempt 2 `succeeded`.

![kill -9 recovery](docs/images/kill-9-recovery.gif)

The recording is generated by [`backend/scripts/record_kill_demo.py`](backend/scripts/record_kill_demo.py),
which runs the real scenario rather than replaying a script — it spawns real workers, really kills
one, and polls Postgres for the outcome. If the recovery ever stops working, the recorder fails
instead of producing a reassuring GIF.
